...
def log(value, status='[GASKEN CUK]', color='[CC]'):
...
def log_replace(value, status='Oprek android', color=''):
...
class inject(object):
    ...
    def start(self):
        try:
            ...
            frontend_domains = open(real_path('/ServerSocks.py')).readlines()
            frontend_domains = filter_array(frontend_domains)
            if len(frontend_domains) == 0:
                self.log('Frontend Domains not found. Please check ServerSocks.py', color='G1')
                return
            self.log('\n'.format(self.inject_host, self.inject_port))
            while True:
                socket_client, _ = socket_server.accept()
                socket_client.recv(9999)
...
class domain_fronting(threading.Thread):
    ...
    def log(self, value, status='SUCCES', 
    color=''):
    ...
    def run(self):
        try:
            ...
            self.log('[CC]TUNGGU sabar...'.format(self.proxy_host, self.proxy_port))
            ...
            self.log('AMBYAR 200 (OK)'.format(self.proxy_host, self.proxy_port), color='[G1]')
        except OSError:
...
G = '\033[36;1m'
print G + '    KUOTA GAMEMAX DENGAN SENTUHAN SCRIPT BERBEDA    \n'
print  ''
mengetik('  -*@*-  ')
mengetik('*********')
print '\033[32;1m'
print('============================================')
print('============================================')
mengetik('DIHARUSKAN PAKE POPON YANG TERBARU')
mengetik('DARI PLAYSTORE Versi:280 YANG SUDAH')
mengetik('UNLIMITED AGAR SUPPORT V.C WHATSAPP')
print('============================================')
print('============================================')
print '\033[31;2m'
mengetik('"NO SNIFF FUCK YOU HARGAI KARYA ORANG LAIN !!!!!"')
print  '\033[33;1m'
mengetik('#### SUB DUKUNG CHANNEL NYUNGSEP OFFICIAL ####')
print '\033[34;1m'
mengetik('psiponnya server portnya 127.0.0.1 &&  8080')
def main():
